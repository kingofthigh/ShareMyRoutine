...
# 운동 기록 작성 
class WkrecordListView(LoginRequiredMixin, ListView):
    model = WkRecord
    template_name = "smr/wkrecord_list.html"
    context_object_name = "wkrecords"
    

    def get_queryset(self):
        return WkRecord.objects.filter(wkout__id=self.kwargs.get('wkout_id'))

    def get_context_data(self, **kwargs):
        context = super().get_context_data(**kwargs)
        context["form"] = WkRecordForm()
        total_vol = WkRecord.objects.filter(wkout__id=self.kwargs.get('wkout_id'))
    ...
    def post(self, request, *args, **kwargs):
        wkout = Wkout.objects.get(id=self.kwargs.get('wkout_id'))
        form = WkRecordForm(request.POST)
        if form.is_valid():
            form.instance.wkout = wkout
            form.save()
        return redirect('wkrecord-list', wkout_id=self.kwargs.get('wkout_id')) 
        # return self.get(request, *args, **kwargs) 
        #--> 새로고침 시 form이 다시 데이터를 가지고 렌더되는 오류
